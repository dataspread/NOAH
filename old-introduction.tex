...
and analysis of data by end-users~\cite{excel-users}. \saj{Studies show that information workers prefer to operate on their data within spreadsheets while shunning enterprise solutions with more advanced analytical features~\cite{chan1996use,raden2005shedding}. One popular joke among those developing business intelligence applications is that  the ``export to excel'' button is the third-most commonly used button from the menu bar, after OK and Cancel~\cite{bakke2016expressive}.}
...
\saj{Scrolling is the action of moving displayed text or graphics up, down, or across on a computer screen in order to view different parts of the spreadsheet.}
...
to provide a \saj{global} context of the user's current location
\saj{being displayed} in the \saj{zoomed in} detailed view~\cite{cockburn2009review}.
...
capturing the
data distribution and \saj{augmenting it with summarization},
...
% \noah leverages {\scshape DataSpread}'s
% underlying spatial mapping of data to ensure coordination between the overview and the spreadsheet so that users can relate the interactions on the overview with the raw data.
