...
scenarios, including XXX\agp{argue for
...
\code{present} (\checkmark: {\em communication of information}, e.g., Rebecca takes a screenshot of \noah to share with her coworkers of the overall availability trends in the context of raw listings), 
...
\emph{Search}            & 
\code{explore} (\checkmark: {\em searching based on characteristics where location is unknown}, e.g.,), 
\code{browse} (\checkmark: {\em searching based on characteristics where location is known}, e.g.,), 
\code{locate} (\checkmark: {\em searching based on entities where location is unknown}, e.g., Rebecca wants to find all entries corresponding to a given city like Chicago), \code{lookup} (\checkmark: {\em searching based on entities where location is known}, e.g.,) \\
\emph{Query}    		&  
\code{identify} (\checkmark: {\em returning the characteristics of entity found during search}, e.g., Rebecca wants to examine Chicago listings and the typical availabilities of listings in Chicago), 
...
\emph{Produce}         & \code{export/save/generate} ($\times$), \code{record} (\checkmark:)\\
\hline
\end{tabular}
}
\vspace{-5pt}
