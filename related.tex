...
As such, the spreadsheet look-and-feel is lost,
and as a result, users
cannot navigate and directly manipulate the raw data
in the way they do in spreadsheets. 
For example, using these tools users cannot \code{browse} subsets of data and \code{lookup} or \code{locate} data points of interest (see Table~\ref{tab:scope}). \agp{previous line problematic: they can... because all these tools support to varying extents the typology}
Even users of such tools want to inspect the underlying data to verify the summary statistics and visualizations~\cite{2017trust}. \agp{this line is problematic too... because the citation is to an AQP tool rather than some general purpose study}
...
The goals of spreadsheets differ from TDA 
