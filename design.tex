...
Histograms are binned aggregates
where each bin represents a collection (group) of data points and the frequency of the data points, \ie \code{COUNT} aggregate, visualizes the density of the corresponding group.} \agp{This previous sentence feels repetitive.} \saj{rephrased. wanted to explain that constructing histogram is a type of binned aggregation.}
\newsaj{In addition to providing high level (\eg densities) and low level (e.g., outliers) details, binned aggregation techniques 
enables multi-granularity visual representation of data
by varying the bin size and
have been deployed in 
interactive visualization of 
large scale datasets, \eg in \emph{imMens}~\cite{liu2013immens}.}. \agp{Clarify what is known from prior work and what
you are proposing. Here, it is intermixed.}
\newsaj{An additional benefit of deploying a binned overview for spreadsheets is a decrease in
...
\newsaj{To generate the overview, we construct an equi-histogram. 
Equi-depth histograms are commonly
used for summarizing the statistical properties
of data, with applications in database systems for query optimization and approximate query processing~\cite{ioannidis2003history} and in data mining applications for distribution fitting in data streams~\cite{mousavi2011fast}, among others.} \agp{only one application?}
\new{The equi-depth histogram 
...
\new{One example of this coordinations is} indicating the navigation attribute on the spreadsheet
