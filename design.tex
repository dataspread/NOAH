...
Navigating back and forth \new{between different subsets of data}
...
\new{Displaying a fine-grained overview improves visual clarity 
while increasing visual discontinuity---users need to scroll through the overview to access distant
subsets of data.
Displaying a coarse-grained overview decreases visual discontinuity 
at cost of reduced visual clarity---the overview may 
span too many data subsets and appear visually cluttered.}
...
\noah supports other interactions \new{atop this multi-granularity overview,}
...
\new{A conventional design for overviews within
popular interfaces is as a spatially partitioned
collection of thumbnails 
on the left of the standard detailed view,
similar to Microsoft Power Point or
Adobe Reader.
...
Multi-granularity representations have been shown to
scale better to 
...
Thus, the multi-granularity overview of \noah
provides an alternative to the aforementioned conventional 
spatially partitioned 
single-granularity representation of the data space, \eg in Power Point, 
by allowing users to control the scale at 
which the overview should be displayed~\cite{cockburn2009review}.
...
\new{The data structure underlying the overview
is a histogram constructed
on the values
in the navigation attribute column.
Histograms result from {\em binned aggregation}---consecutive data points are grouped into bins (or groups),
where each bin represents a collection (group) 
of data points and is associated with a count aggregate,
capturing the number of data points that fall in that group.
In addition to providing high level (\eg densities) 
and low level (e.g., outliers) details, 
binned aggregation techniques 
enable a multi-granularity visual representation of data
by varying the bin size and
have therefore been deployed in 
interactive visualization of 
large scale datasets, \eg in \emph{imMens}~\cite{liu2013immens}.
An additional benefit of a binned overview for spreadsheets 
is a decrease in
visual discontinuity during navigation. As users are able to 
view an overview 
that fits in the computer screen, they can 
...
\new{To generate the overview, we construct an equi-histogram. 
Equi-depth histograms are commonly
used for summarizing statistical properties
of data, with applications in database systems 
for query optimization and approximate 
query processing~\cite{ioannidis2003history} 
and in data mining applications for distribution fitting in data streams~\cite{mousavi2011fast}, among others.
...
is discoverability \new{of the zoom operation}~\cite{cockburn2009review}.
...
\new{One example of this coordination is} indicating the navigation attribute on the spreadsheet
...
focus---\emph{\new{to ensure consistency between the overview and the spreadsheet,} any interaction on
either interface
...
\new{The aggregate column can employ any statistical
...
The \noah front-end is built with HTML/CSS/JS technologies along with the D3 framework~\cite{d3} for generating charts. We explain the \noah architecture in detail in the Appendix.
