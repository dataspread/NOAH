...
\para{DC6. Display contextual and historical information regarding users' navigation}
...
provide an alternative to a single-granularity
spatial partition of the data space,
by varying the scale at which the data is displayed,
enabling multiple views at varying granularities~\cite{cockburn2009review}.
...
Moreover, users can also hide the overview if required.}

\saj{As explained earlier,
we divide each granularity
of the overview into bins.
The highest (coarsest) level of the granularity
provides a global view with less visual clarity
and reduced visual discontinuity,
As the bins become more fine-grained, visual clarity improves while visual discontinuity increases.
...
and quickly navigate to the desired subset of data.
...
The data structure underlying the overview
...
\toappendix{\agp{We don't have time but for the future, this is
something that would have been great to describe the algorithm on
for technical depth.}}
...
Users can choose to view the result of formulae
