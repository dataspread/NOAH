...
\saj{\noah was designed to address the navigational challenges in spreadsheets. As such, the goal of the study was to evaluate whether \noah enhances the navigational capabilities of traditional spreadsheets. Furthermore, we wanted understand how the presence of the features introduced by a plugin like \noah affects a spreadsheet user's navigation experience. In this study, we compare a \noah-integrated spreadsheet with a typical, popular one, Excel, for various navigational tasks in the spreadsheet domain. Similar domain specific evaluations have been performed for measuring the performance of various  overview+detail interfaces, \eg database browsing~\cite{north2000snap} or 
tree navigation~\cite{beard1990navigational}. As explained in Section~\ref{sec:related}, the goals of spreadsheets and the TDA tools are quite different. Therefore, we did not consider TDA tools for the comparative study.}
...
\saj{How did the addition of an overview like \noah affect a spreadsheet users navigation performance?}
...
\saj{How did \noah and it's components impact spreadsheet users'
...
steer (two tasks), compare (two tasks), identify (one task), and customize (one task), representing the Table~\ref{tab:scope} uses cases \code{summarize}, \code{compare}, \code{identify}, and \code{create}, respectively. All of the tasks except the customize task, are representative of the navigation interactions required for the most frequently issued spreadsheet formulae
...
\kgk{Our study has several limitations that can be strengthened by future larger-scale and more fine-grained studies. One limitation arises from our participant demographics.  We conducted the study with $20$ participants from four different backgrounds. Our participant pool demographics partially represent the demographics of the general audience intended for \noah.
...
\saj{Another limitation of the study is that we only compared the performance of a \noah-integrated spreadsheet with a traditional spreadsheet. We did not evaluate specific spreadsheet features like pivot table and \code{SUBTOTAL} due to their limitations in the types of interactions supported (see Section~\ref{sec:related}). Moreover, we wanted the participants to utilize the typical spreadsheet operations that they are comfortable with to complete those tasks and see how introduction of \noah affected their navigation experience.} 
